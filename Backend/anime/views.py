...
class get_3_years_season(APIView):
    def get(self, request):
        season = Season.objects.order_by('-id')[:12] #1 năm có 4 mùa 4 năm 12 mùa
...
class get_animeEspisode_newest(APIView):
    def get(self,request,count):
        ...
        serializerAnimeEspisode = AnimeEspisodeSerializer(animeEspisode, many = True)
        ...
        for espisode in anime_espisode_data:
            ...
            combined_data.append({**espisode, **serializerAnime.data, 'images': images, 'name': AnimeNameSerializer(name).data['name'], 'types': get_type_anime(espisode['animeid'])})
...
def get_anime_current_espisode (anime_id):
    try:
        currentEspisode = AnimeEspisode.objects.filter(animeid = Anime.objects.get(id = anime_id)).aggregate(max_espisode = Max('espisode'))['max_espisode']
...
class get_anime_newest(APIView):
    def get(self, request,count):
        try:
            ...
            animes = Anime.objects.filter(showtimeid__airstart__lte=current_time, isdeleted=False).order_by('-showtimeid__airstart')[:count]
            combined_data = []
            for anime in animes:
                ...
                anime_data['currentEspisode']= get_anime_current_espisode(anime.id)
...
class get_anime_upcoming(APIView):
    def get(self, request,count):
        try:
            current_time = timezone.now()
            animes = Anime.objects.filter(showtimeid__airstart__gt=current_time, isdeleted = False).order_by('showtimeid__airstart')[:count]
            combined_data=[]
            for anime in animes:
                anime_data = AnimeSerializer(anime).data
                anime_data['types'] = get_type_anime(anime.id)
...
class get_anime_by_name(APIView):
    def get(self, request, anime_name):
        try:
            ...
            animeName = AnimeName.objects.get(name__icontains = anime_name_string.strip())
            anime = animeName.animeid
            anime_data = AnimeSerializer(anime).data
            anime_data['images'] = imagesView.get_all_image_anime(anime.id)
            anime_data['types'] = get_type_anime(anime.id)
            anime_data['name']= AnimeNameSerializer(animeName).data['name']
            anime_data['EspisodeImg'] = imagesView.get_all_anime_espisode_image(anime.id)
...
class add_anime_espisode_video(APIView):
    def post(self, request):
        try:
            ...
            blob_path = f"anime/{valid_name}/Espisode/{espisode}/{espisode}"
            ...
            # Tạo đường dẫn công khai của video
            public_url = f"https://storage.googleapis.com/animewebsite-2f080.appspot.com/{blob_path}"
            
            return Response({"video_url": public_url})
        except Exception as e:
            print(f'Error adding video: {e}')
            return Response({'message': 'Error adding video'})
